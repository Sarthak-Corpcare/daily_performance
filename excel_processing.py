...
from config import RAW_TO_TEMPLATE_HEADER_MAP, MONTH_GROUPED_HEADERS, LOGO_FILENAME, \
    template_file,RATING_HEADERS, SHEETS_TO_IGNORE, button_border, center_align, light_brown_fill, \
    back_button_font, back_button_fill, no_fill
from utils import update_as_on_date, find_header_row, get_month_id_for_column, is_date_like, \
    get_parent_header_for_column, is_meaningful_data, auto_fit_columns
...
def create_styled_homepage(workbook, SHEETS_TO_IGNORE, LOGO_FILENAME):
    ...
    data_sheets = sorted(
        [s for s in workbook.sheetnames if s not in SHEETS_TO_IGNORE and s.strip().lower() != 'disclaimer'])
...
def process_sheet(raw_wb, template_wb, SHEETS_TO_IGNORE):
    ...
    for sheet_name in raw_wb.sheetnames:
        ...
        for row_num in range(data_header_row_raw + 1, raw_sheet.max_row + 2):
            ...
            handling_standard_column(dest_col_map, raw_col_map, raw_sheet, row_num, latest_month_id, older_month_id,
                                     any_older_data_used,
            ...
            handling_AUM(aum_dest_col, latest_aum_col_raw, older_aum_col_raw, raw_sheet, row_num, template_sheet,
                         template_row_index)
        ...
        # --- FORMAT & LEGEND ---
        format_and_legend(template_sheet, data_header_row_template, start_row_template, rows_on_this_sheet,
                          aum_dest_col,
                          any_older_data_used, older_date_for_legend, older_month_id)


def handling_standard_column(dest_col_map, raw_col_map, raw_sheet, row_num, latest_month_id, older_month_id,
                             any_older_data_used,
                             template_sheet, template_row_index):
...
def handling_AUM(aum_dest_col, latest_aum_col_raw, older_aum_col_raw, raw_sheet, row_num, template_sheet,
                 template_row_index):
    # B) AUM
    if aum_dest_col:
...
def handle_expense_ration(exp_dates, dest_col_map, raw_sheet, row_num, template_sheet, template_row_index):
    ...
    if exp_dates and (dest_col := dest_col_map.get("Direct Expense Ratio")):
        ...
        if her_value is None and len(exp_dates) > 1 and (
                val := raw_sheet.cell(row=row_num, column=exp_dates[1][0]).value) is not None and str(
            val).strip() != "":
            her_value = val
            her_is_older = True
            any_older_data_used = True
